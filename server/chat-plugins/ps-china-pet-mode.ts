...
function petBox(petUser: PetUser, target: string): string {
	...
	const boxTitle = `${st('用户ID')} ${petUser.id}&emsp;${st('徽章数')} ${petUser.property['badges'].length}`;
	...
	return `|uhtml|pet-box-show|<div style="height: 300">${boxDiv}${pokeDiv}</div>`;
}
